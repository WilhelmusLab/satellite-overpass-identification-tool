...
def terra_20250515_events(rate_limited_get_data, credentials, domain):
    ...
    if not terra_data or "EPOCH" not in terra_data[0]:
        pytest.skip(
            "terra TLE data unavailable (likely query throttled by space-track.org)"
        )
...
def test_process_passes_with_representative_terra_events(
    ...
    for expectation, actual in zip(expected_passes, passes):
        ...
        assert (
            abs(
                (
                    dt.datetime.strptime(actual_time, "%H:%M:%S")
                    - dt.datetime.strptime(expected_time, "%H:%M:%S")
                ).total_seconds()
            )
            <= pass_times_tolerance_minutes * 60
